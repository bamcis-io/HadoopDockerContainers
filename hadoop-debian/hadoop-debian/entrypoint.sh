...
if [ ! -z "${SLAVE_NODES// }" ] || [ ${ROLE,,} = "slave" ]; then
	...
	if [ -e "${HADOOP_HOME}/etc/hadoop/core-site.xml.template" ]; then
		su --login "${HADOOP_USER}" --command "/bin/cp -f ${HADOOP_HOME}/etc/hadoop/core-site.xml.template ${HADOOP_HOME}/etc/hadoop/yarn-site.xml"
	else
		su --login "${HADOOP_USER}" --command "/bin/cp -f ${HADOOP_HOME}/etc/hadoop/core-site.xml ${HADOOP_HOME}/etc/hadoop/yarn-site.xml.template"
	fi

	sed -i "s/localhost/${MASTER_NODE}/g" "${HADOOP_HOME}/etc/hadoop/yarn-site.xml"
